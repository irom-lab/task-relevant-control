...
%init_dist(sub2ind([Parameters.Runway Parameters.Runway Parameters.Height 2], 1, 1, Parameters.Height, 1)) = 1;
%init_dist(sub2ind([Parameters.Runway Parameters.Runway Parameters.Height 2], 1, 1, Parameters.Height, 2)) = 1;
%init_dist(sub2ind([Parameters.Runway Parameters.Runway Parameters.Height 2], 1, 1, Parameters.Height - 1, 1)) = 1;
%init_dist(sub2ind([Parameters.Runway Parameters.Runway Parameters.Height 2], 1, 1, Parameters.Height - 1, 2)) = 1;
...
%Problem.SolverOptions.InitPolicy.CodeGivenState = repmat(eye(Parameters.Length + 1), 1, 1, Parameters.Horizon) + 0.01;
%Problem.SolverOptions.InitPolicy.CodeGivenState = Problem.SolverOptions.InitPolicy.CodeGivenState ./ sum(Problem.SolverOptions.InitPolicy.CodeGivenState);
%Problem.SolverOptions.InitPolicy.InputGivenCode = controller;
...
% Problem.Controller.Estimator = 'Code';
% 
% fprintf('Running N = %d simulations w/ Bayes estimator...\n', samples);
% 
% for i = 1:samples
%     [~, c3(:, i)] = sim_meas_uncertainty(Problem,  [0.3; 0.4; 0; 0.3; 0], meas_rate, init_dist);
% end
% 
% fprintf('\tFinished in %fs\n', time);